...
def test_rrf_favors_item_ranked_high_in_both_lists():
    vector_results = [
        {"stock_id": 1, "ticker": "A", "name_kr": "가", "content": "..."},
        {"stock_id": 2, "ticker": "B", "name_kr": "나", "content": "..."},
    ]
    keyword_results = [
        {"stock_id": 1, "ticker": "A", "name_kr": "가", "content": "..."},
        {"stock_id": 3, "ticker": "C", "name_kr": "다", "content": "..."},
    ]

    fused = HybridStockSearch._reciprocal_rank_fusion(vector_results, keyword_results)

    assert fused[0].stock_id == 1  # 두 검색 모두에서 1위 -> 최상위
    assert {c.stock_id for c in fused} == {1, 2, 3}


def test_rrf_handles_item_in_only_one_list():
    vector_results = [{"stock_id": 1, "ticker": "A", "name_kr": "가", "content": "..."}]
    keyword_results = []
